<template>
  <div class="center">
        ...
        <v-form>
          <v-container>
            <v-row>
              <v-col cols="12" md="6">
                <v-numeric
                  label="Ilość posiadanego alkoholu"
                  required
                  clearable="true"
                  precision="0"
                  suffix="ml."
            ...
            <v-card-action justify="space-around" class="rounded-b-lg">
              ...
              <v-btn block class="" color="error" depressed @click="clear()">
                Wyczyść
              </v-btn>
            </v-card-action>
          </v-container>
          ...
          <v-container>
            <v-row>
              <v-col cols="12" md="6">
                ...
                ></v-numeric>
                <v-numeric
                  label="Moc posiadanego spirytusu"
                  clearable="true"
                  min="0"
                  max="100"
                  precision="1"
                  suffix="%"
                  v-model="moc_spirytusu"
                  required
                ></v-numeric>
              </v-col>
            ...
            <v-card-action justify="space-around" class="rounded-b-lg">
              ...
              <v-btn block color="error" depressed @click="clear()">
                Wyczyść
              </v-btn>
            </v-card-action>
          </v-container>
    ...
    <div class="my-2 mx-2 rounded-lg">
      
      <v-row>
        <v-col cols="12" md="6">
          
          <v-btn block color="success" v-on:click="isShown = !isShown"
            >Ile potrzebujesz wódki ?</v-btn
          >
...
export default {
  ...
  methods: {
    ...
    ilosc_wody() {
      this.result_wody = parseFloat(
        (this.ilosc * this.moc) / this.moc_doc - this.ilosc
      );
      this.result_wody =
        this.result_wody +
        (parseFloat(this.result_wody) + parseFloat(this.ilosc)) * 0.03;
      return this.result_wody || 0;
    },
    ilosc_alkoholu() {
      this.result_alko =
        parseFloat((this.ilosc * this.moc) / this.moc_doc - this.ilosc) +
        parseFloat(this.ilosc);
    ...
    potrzeba_spirytusu() {
      this.result_potrzeba_spirytusu =
        parseFloat(this.ile_wodki * this.moc_wodki) / this.moc_spirytusu;
      return this.result_potrzeba_spirytusu || 0;
    },

    potrzeba_wody() {
      this.result_potrzeba_wody =
        parseFloat(this.ile_wodki) -
        parseFloat((this.ile_wodki * this.moc_wodki) / this.moc_spirytusu) +
        this.result_kontrakcja2;
